...
module.exports = class Core {
  ...
  adjustDotPosition(value) {
    ...
    result = `${result.substring(0, fractionDigits)}.${result.substring(
      fractionDigits
    )}`;
  ...
  checkNumberStart(value, separator) {
    ...
    return result[0] === separator
      ? `${this.args.fixed ? "0" : "_"}${result}`
      : result;
  }
  ...
  checkSuffix(value) {
    ...
    const currentLastSuffix = value.substring(
      lastIndex - this.args.suffix.length + 1,
      lastIndex + this.args.suffix.length
    );
    const currentLastButOneSuffix = value.substring(
      lastButOneIndex - this.args.suffix.length + 1,
      lastButOneIndex + this.args.suffix.length
    );
    ...
    switch (this.args.suffix) {
      ...
      case currentLastButOneSuffix:
        ...
        result = `${start}${value.substring(
          value.length + this.args.suffix.length + 1,
          lastButOneIndex + this.args.suffix.length
        )}.`;
        break;
      default:
  ...
  emptyOrInvalid() {
    return `${this.completer()}${this.args.decimalSeparator}${this.completer(
  ...
  mask(value) {
    ...
    if (!isNaN(this.removeCompleter(result, completer))) {
      ...
      result = this.addCompleter(
        result || "",
        completer,
        this.args.fractionDigits
      );
      result =
        this.args.fractionDigits >= result.length
          ? `${completer}${result}`
          : result;
    ...
    return `${!this.args.negativeSignAfter && negative ? "-" : ""}${result}${
      this.args.negativeSignAfter && negative ? "-" : ""
    }`;
  }
  ...
  onlyNumber(value) {
    const hasDecimalSeparator = value
      .toString()
      .indexOf(this.args.decimalSeparator);
    ...
    for (let i = value.length - 1; i >= 0; i--) {
      if (isFinite(value[i]) || (!this.args.fixed && value[i] === "_")) {
        ...
        !putDecimalSeparator &&
        value[i] === this.args.decimalSeparator
      ) {
  ...
  removeSuffix(value) {
    const position = value.indexOf(
      this.args.suffix,
      value.length - this.args.suffix.length
    );
    ...
    if (position !== -1) {
      ...
      value =
        start +
        value.substring(
          start.length + this.args.suffix.length - 1,
          value.length - 1
        );
    }
