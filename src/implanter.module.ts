module.exports class Implanter = {
  ...
  addMask(input, core) {
    input.addEventListener("input", (e) => {
      ...
      let newCaretPosition = this.indexMove(
        newValue,
        oldValue,
        oldCaretPosition
      );
      ...
      if (input.maskArgs.cursor === "start") {
        ...
        input.maskArgs.cursor === "end" ||
        (input.maskArgs.cursor === "move" && oldValue.length <= 1)
      ) {
